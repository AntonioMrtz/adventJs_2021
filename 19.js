function learn(time, courses) {
    ...
    for (let i = 0; i < courses.length; i++) {
        ...
        for (let j = i+1; j < courses.length; j++){
            ...
            if(courses[i]+courses[j]<=time && courses[i]+courses[j]>selected_courses.time){

                selected_courses.time=courses[i]+courses[j];
                selected_courses.index=[i,j];
    ...
    if(selected_courses.time==-1){
        ...
        console.log(selected_courses.index)
    
        return selected_courses.index
